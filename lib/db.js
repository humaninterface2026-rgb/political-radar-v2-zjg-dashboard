...
(function (root) {
  ...
  // -----------------------------------------------------------------------
  async function recentComments(platform, limit, hoursBack, subject) {
    ...
    while (out.length < limit) {
      ...
      let q = c
        ...
        .select('comment_id, subject, author, text, signal, signal_zh, signal_score, time_text, url, kind, first_seen_at')
        .eq('platform', platform);
      if (subject) q = q.eq('subject', subject);   // 按政治人物（張嘉郡/劉建國）篩
    ...
    // first_seen_at 保留：前端用它做「最新一批（＝最新貼文快照）」的篩選
    return out;
  }
  ...
  // 張嘉郡 only 變體（_self_filter）— 給「張嘉郡聲量」卡片 + 趨勢圖用
  async function dashboardMetricsSelf(hoursBack) {
    return _cachedRpc('dashboard_metrics_self', { hours_back: hoursBack || 24 });
  }
  async function dashboardByHourSelf(hoursBack) {
    return _cachedRpc('dashboard_by_hour_self', { hours_back: hoursBack || 24 });
  }
  ...
  root.LxyDB = {
    ...
    dashboardMetricsSelf:     dashboardMetricsSelf,
    dashboardByHourSelf:      dashboardByHourSelf,
